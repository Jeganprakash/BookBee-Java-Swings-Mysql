...
public class View extends JFrame implements ActionListener {
    ...
    private ImageIcon setImage(Object imageName, int height, int width) {
        BufferedImage myPicture = null;
        try {
            if (imageName instanceof String) {
                myPicture = ImageIO.read(new File("images/" + (String) imageName));
            } else {
                myPicture = ImageIO.read((InputStream) imageName);
            }
            ImageIcon imageIcon = new ImageIcon(myPicture); // load the image to a imageIcon
            Image image = imageIcon.getImage(); // transform it
            Image newimg = image.getScaledInstance(height, width, java.awt.Image.SCALE_SMOOTH); // scale it the smooth
                                                                                                // // way
            imageIcon = new ImageIcon(newimg); // transform it back
            return imageIcon;
        } catch (final IOException e) {
            System.out.println("Problem in image setting" + imageName);
            e.printStackTrace();
        }
        return null;
